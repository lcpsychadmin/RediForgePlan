...
const planningViewToTab: Record<string, number> = {
  ...
  // Collect all cycles with dates
  const allCycles: any[] = programs.flatMap(prog =>
    (mockCycles[prog.id] || []).map((c: any) => ({ ...c, programName: prog.name, programAccent: prog.accentColor || '#5B67CA' }))
  );
  ...
  const rangeStart = allDates.length ? new Date(Math.min(...allDates.map(d => d.getTime()))) : today;
  const rangeEnd = allDates.length ? new Date(Math.max(...allDates.map(d => d.getTime()))) : new Date(today.getFullYear(), today.getMonth() + 6, 1);
  // Pad 2 weeks on each side
  rangeStart.setDate(rangeStart.getDate() - 14);
  rangeEnd.setDate(rangeEnd.getDate() + 14);
  const totalDays = Math.max(1, Math.ceil((rangeEnd.getTime() - rangeStart.getTime()) / 86400000));

  const dateToX = (d: Date) => ((d.getTime() - rangeStart.getTime()) / 86400000 / totalDays) * 100;

  // Generate month header ticks
  const months: { label: string; left: number }[] = [];
  const cur = new Date(rangeStart.getFullYear(), rangeStart.getMonth(), 1);
  while (cur <= rangeEnd) {
    months.push({ label: cur.toLocaleDateString('en-US', { month: 'short', year: '2-digit' }), left: dateToX(cur) });
    cur.setMonth(cur.getMonth() + 1);
  }
  const todayLeft = dateToX(today);

  const openEdit = (c: any) => {
    setEditingId(c.id);
    setEditStart(c.startDate ? c.startDate.substring(0, 10) : '');
    setEditEnd(c.endDate ? c.endDate.substring(0, 10) : '');
  };

  const saveEdit = async () => {
    if (!editingId) return;
    setSaving(true);
    try { await onSaveCycleDates(editingId, editStart, editEnd); } finally { setSaving(false); setEditingId(null); }
  };
  ...
  return (
    <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Box>
          <Typography variant="h5" sx={{ fontWeight: 700 }}>Roadmap</Typography>
          <Typography variant="caption" color="text.secondary">High-level timeline for programs and mock cycles. Click any bar to edit dates.</Typography>
        </Box>
      </Box>

      {allCycles.length === 0 ? (
        <Box sx={{ p: 4, textAlign: 'center', color: 'text.disabled' }}>
          <Typography>No mock cycles with dates defined. Add dates in the Structure tab.</Typography>
        </Box>
      ) : (
        <Box sx={{ overflowX: 'auto' }}>
          <Box sx={{ minWidth: 800, position: 'relative' }}>
            {/* Month header */}
            <Box sx={{ position: 'relative', height: 28, mb: 0.5, borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
              {months.map((m, i) => (
                <Typography key={i} variant="caption" sx={{ position: 'absolute', left: `${m.left}%`, fontSize: '0.65rem', color: 'rgba(255,255,255,0.4)', fontWeight: 600, whiteSpace: 'nowrap', transform: 'translateX(-2px)' }}>
                  {m.label}
                </Typography>
              ))}
              {/* Month grid lines */}
              {months.map((m, i) => (
                <Box key={i} sx={{ position: 'absolute', left: `${m.left}%`, top: 0, bottom: 0, width: '1px', backgroundColor: 'rgba(255,255,255,0.06)' }} />
              ))}
              {/* Today line */}
              {todayLeft >= 0 && todayLeft <= 100 && (
                <Box sx={{ position: 'absolute', left: `${todayLeft}%`, top: 0, bottom: -999, width: '1px', backgroundColor: 'rgba(91,103,202,0.6)', zIndex: 2 }} />
              )}
            </Box>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
              {programs.map(prog => {
                const cycles = mockCycles[prog.id] || [];
                if (cycles.length === 0) return null;
                const accent = prog.accentColor || '#5B67CA';
                return (
                  <React.Fragment key={prog.id}>
                    {/* Program label row */}
                    <Box sx={{ py: 0.5, px: 1, display: 'flex', alignItems: 'center', gap: 0.75 }}>
                      <Box sx={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: accent, flexShrink: 0 }} />
                      <Typography variant="caption" sx={{ fontWeight: 700, color: accent, textTransform: 'uppercase', letterSpacing: '0.06em', fontSize: '0.65rem' }}>{prog.name}</Typography>
                    </Box>
                    {cycles.map((c: any) => {
                      const hasStart = !!c.startDate;
                      const hasEnd = !!c.endDate;
                      const xStart = hasStart ? dateToX(new Date(c.startDate)) : 0;
                      const xEnd = hasEnd ? dateToX(new Date(c.endDate)) : xStart + 5;
                      const width = Math.max(2, xEnd - xStart);
                      const isEditing = editingId === c.id;
                      return (
                        <Box key={c.id} sx={{ position: 'relative', height: 40, mb: 0.25 }}>
                          {/* Background track */}
                          <Box sx={{ position: 'absolute', left: 0, right: 0, top: '50%', transform: 'translateY(-50%)', height: '1px', backgroundColor: 'rgba(255,255,255,0.05)' }} />
                          {/* Bar */}
                          {(hasStart || hasEnd) && (
                            <Box
                              onClick={() => openEdit(c)}
                              sx={{
                                position: 'absolute', left: `${xStart}%`, width: `${width}%`,
                                top: '50%', transform: 'translateY(-50%)', height: 28,
                                borderRadius: 1.5, cursor: 'pointer',
                                backgroundColor: `${accent}33`, border: `1.5px solid ${accent}99`,
                                display: 'flex', alignItems: 'center', px: 1.25,
                                '&:hover': { backgroundColor: `${accent}55`, borderColor: accent },
                                transition: 'all 0.15s',
                              }}
                            >
                              <Typography sx={{ fontWeight: 600, fontSize: '0.72rem', color: accent, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                {c.name}
                              </Typography>
                            </Box>
                          )}
                          ...
                          {!hasStart && !hasEnd && (
                            <Box onClick={() => openEdit(c)} sx={{ position: 'absolute', left: '4px', display: 'flex', alignItems: 'center', top: '50%', transform: 'translateY(-50%)', height: 28, px: 1.25, borderRadius: 1.5, border: `1px dashed rgba(255,255,255,0.2)`, cursor: 'pointer', '&:hover': { borderColor: accent } }}>
                              <Typography sx={{ fontSize: '0.72rem', color: 'text.disabled' }}>{c.name} — click to set dates</Typography>
                            </Box>
                          )}
                        </Box>
                      );
      ...
      {/* Edit dialog */}
      {editingId && (() => {
        const c = allCycles.find(x => x.id === editingId);
        return (
          <Box sx={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 1400, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
            onClick={(e) => { if (e.target === e.currentTarget) setEditingId(null); }}>
            <Box sx={{ backgroundColor: '#141e35', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 2, p: 3, width: 360, display: 'flex', flexDirection: 'column', gap: 2 }}>
              <Box sx={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', mx: -3, mt: -3, px: 3, py: 1.5, borderRadius: '8px 8px 0 0' }}>
                <Typography sx={{ fontWeight: 700, fontSize: '1rem' }}>Edit Cycle Dates</Typography>
                <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.7)' }}>{c?.name}</Typography>
              </Box>
              ...
              <Box sx={{ display: 'flex', gap: 1, justifyContent: 'flex-end' }}>
                <Button size="small" onClick={() => setEditingId(null)} sx={{ textTransform: 'none' }}>Cancel</Button>
              ...
              {!selectedItem ? (
                ...
                <>
                  {selectedItem.type === 'project' || selectedItem.type === 'processArea' ? (() => {
                    ...
                    const rawProcessAreaFilter = selectedItem.type === 'processArea'
                      ? (selectedItem.area || '')
