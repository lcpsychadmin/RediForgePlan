...
type SelectableItem =
  ...
  | { type: 'deliverableProcessArea'; projectId: string; cycleId?: string; deliverableId: string; area: string }
...
const PLAN_DELIVERABLE_NODES: Array<{
  ...
  { id: 'mockCriteria', label: 'Entry/Exit Criteria', targetView: 'plan', accentColor: '#66BB6A', icon: 'fa-list-check' },
...
const RoadmapView: React.FC<RoadmapViewProps> = ({ programs, mockCycles, projectsByMockCycle, onSaveCycleDates, forcedProjectName, hideFilters = false }) => {
  ...
  const [lastRoadmapInteraction, setLastRoadmapInteraction] = React.useState('');
  ...
  const TimelineBar = ({ startDate, endDate, color, label, subtitle, onClick, dashed = false, onStartDrag, onEndDrag }: {
    startDate?: string; endDate?: string; color: string; label: string; subtitle?: string;
    ...
    const HANDLE = 8;
    const openEditFromTimeline = (e?: React.MouseEvent) => {
      e?.stopPropagation();
      onClick?.();
    };
    if (!hasS && !hasE) return (
      <Box
        ...
        onDoubleClick={openEditFromTimeline}
  ...
  const openEditItem = (item: RoadmapItem) => {
    setLastRoadmapInteraction(`Edit requested: ${item.name || item.type}`);
                            ...
                            <React.Fragment key={c.id}>
                              ...
                              <Box sx={{ opacity }}>
                                <TimelineBar startDate={dispStart} endDate={dispEnd} color={accent} label={c.name}
                                  subtitle={`${proj.programName} · ${proj.name}`}
                                  onClick={() => {
                                    setLastRoadmapInteraction(`Edit requested: ${c.name}`);
                                    setEditCycle(c);
                                    setEditStart(c.startDate?.substring(0, 10) || '');
                                    setEditEnd(c.endDate?.substring(0, 10) || '');
                                  }}
...
const ProjectsPage: React.FC<ProjectsPageProps> = ({ sectionMode = 'execution', planningView }) => {
  ...
  const lastHierarchyPersistPayloadRef = React.useRef('');
  ...
  const [newCriteriaApprovalTitle, setNewCriteriaApprovalTitle] = useState('');
  const [newCriteriaApprovalAssignee, setNewCriteriaApprovalAssignee] = useState('');
  const [isCreatingCriteriaApproval, setIsCreatingCriteriaApproval] = useState(false);
  ...
  const activeProjectId = canAccessInventory && tabValue === 1
    ...
    : (selectedItem?.type === 'project'
      ...
      : selectedItem?.type === 'deliverableProcessArea'
        ? selectedItem.projectId
  ...
  const cycleEntryCriteriaRows =
    Array.isArray(cycleEntryCriteriaDraft) && cycleEntryCriteriaDraft.length > 0
      ...
      : MOCK_CYCLE_ENTRY_CRITERIA_DEFAULTS;
  const cycleExitCriteriaRows =
  ...
  const renderHierarchyIcon = (level: HierarchyLevel, color: string, size: string, overrideKey?: string, overrideArea?: string) => {
    ...
    // Delegate to renderIconChoice which handles MUI, FA, and all new icon types.
    return renderIconChoice(iconChoice, color, size);
  };
      ...
      try {
        ...
        if (activeCycleId && activeProjectId) {
          ...
          for (const name of missingNames) {
            try {
              const syncRes = await apiClient.post(`/api/tasks/groups/cycle/${activeCycleId}`, { name: name.trim(), processArea: null });
  ...
  const confirmAddToInv = async () => {
    ...
    try {
      ...
      // Navigate to the inventory page so the user sees the added item
      if (sectionMode === 'planning') navigate('/planning/inventory');
    } catch (e: any) {
    ...
    apiClient.get(`/api/applications/data-definitions/object/${globalObj.id}`)
      .then(async (ddRes) => {
        ...
        if (sourceApplicationId) {
          const matchedSource = options.find((opt: any) => opt.applicationId === sourceApplicationId);
          if (matchedSource) {
            setInvDialogSourceAppId(matchedSource.id);
          }
        }
  ...
  const startSubObjectCreateFromParent = (parentItem: any) => {
    ...
    setProjectInventoryItem({
      ...
      targetApplicationId: parentItem.targetApplicationId || '',
      sourceApplicationId: parentItem.sourceApplicationId || '',
    });
  };
  ...
  useEffect(() => {
    ...
    const estimationNodeSelected = (
      (selectedItem?.type === 'deliverable' || selectedItem?.type === 'deliverableProcessArea')
      && selectedItem.deliverableId === 'designBuildEstimation'
    );
    if (!estimationNodeSelected) return;
  ...
  useEffect(() => {
    ...
    if (selectedItem?.type === 'deliverable') {
      if (selectedItem.deliverableId.startsWith(MOCK_CRITERIA_CYCLE_NODE_PREFIX)) {
        ...
        setCycleExitCriteriaDraft(normalizeCriteriaDraft(cycle?.exitCriteriaItems, MOCK_CYCLE_EXIT_CRITERIA_DEFAULTS));
        if (cycleId) {
          loadCycleWorkflowStatus(cycleId).catch(() => {});
        } else {
          setCycleWorkflowEvaluation(null);
        }
      }
    ...
    if (selectedItem?.type === 'deliverableProcessArea') {
      loadProjectWorkflowRoles(selectedItem.projectId).catch(() => {});
      setCycleWorkflowEvaluation(null);
      return;
    }
  ...
  const getSelectedItemDetails = () => {
    ...
    } else if (selectedItem.type === 'deliverableProcessArea') {
      for (const cycleId in projectsByMockCycle) {
        const project = projectsByMockCycle[cycleId]?.find(p => p.id === selectedItem.projectId);
        if (project) return project;
      }
  ...
  const calcEndDateWithContext = (taskId: string, startDate: string, duration: number, task: any, tasksSnapshot: any[]): string => {
    ...
    const siblings = tasksSnapshot
      ...
      .sort((a, b) => {
        ...
        // Tiebreak by template order so "Transformation" sorts before "PreLoad Validation"
  ...
  const applyAutoSchedule = (tasks: any[]): any[] => {
    ...
    for (const siblings of Object.values(byObject)) {
      ...
      const sorted = [...siblings].sort((a, b) => {
        ...
        // Tiebreak by default task template order so e.g. "Transformation"
        // sorts before "PreLoad Validation" (P < T alphabetically but wrong order).
        const aIdx = defaultTaskOrder.indexOf(a.name);
        const bIdx = defaultTaskOrder.indexOf(b.name);
        const aOrd = aIdx === -1 ? 9999 : aIdx;
        const bOrd = bIdx === -1 ? 9999 : bIdx;
        if (aOrd !== bOrd) return aOrd - bOrd;
        // Final fallback: creation time if available, else name
        if (a.createdAt && b.createdAt) return new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime();
        return (a.name || '').localeCompare(b.name || '');
      });
    ...
    >
      ...
      <Box sx={{ display: 'flex', flex: 1, overflow: 'hidden', position: 'relative' }}>
        ...
        >
          ...
          <Box sx={{ flex: 1, overflowX: 'hidden', overflowY: 'auto', pt: 1, display: isPlanningMaintainTab ? 'none' : 'block' }}>
            {programs.length === 0 ? (
              ...
              <List sx={{ p: 0, overflowX: 'hidden', width: '100%', maxWidth: '100%' }}>
                                    ...
                                    <Box sx={{ display: 'grid', gap: 0.5, pb: 0.5 }}>
                                      {PLAN_DELIVERABLE_NODES.filter((candidate) => !candidate.parentId).map((node) => {
                                        ...
                                        const processAreaRoleNodes = isProcessAreasNode
                                          ...
                                          : [];
                                        const objectInventoryProcessAreas = isObjectInventoryNode
                                          ? getProcessAreasForProjectCycle(firstCycleProject.id, firstCycle?.id)
                                          : [];
                                        ...
                                        return (
                                          <Box key={`${projectGroupKey}-${node.id}`}>
                                            ...
                                            >
                                              ...
                                              <Typography variant="caption" sx={{ fontWeight: isNodeSelected ? 700 : 500, color: isNodeSelected ? nodeAccent : 'inherit', flex: '1 1 auto', minWidth: 0, maxWidth: '100%', width: 0, display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                {node.label}
                                              </Typography>
                                              {node.isFuture && (
                                            ...
                                            {isObjectInventoryNode && objectInventoryProcessAreas.length > 0 && isNodeExpanded && (
                                              <Box sx={{ ml: 2.2, mt: 0.2, display: 'grid', gap: 0.35 }}>
                                                {objectInventoryProcessAreas.map((area) => {
                                                  const areaSelected = selectedItem?.type === 'deliverableProcessArea'
                                                    && selectedItem.projectId === firstCycleProject.id
                                                    && selectedItem.deliverableId === node.id
                                                    && selectedItem.area === area;
        ...
        <Box sx={{ flex: 1, overflowY: 'auto', p: { xs: 1.25, sm: 3 }, minWidth: 0 }}>
                  ...
                  {(selectedItem.type === 'project' && !isPlanningPlanHierarchy) || selectedItem.type === 'processArea' || selectedItem.type === 'deliverable' || selectedItem.type === 'deliverableProcessArea' ? (() => {
                    ...
                    const selectedAreaLabel = (selectedItem.type === 'processArea' || selectedItem.type === 'deliverableProcessArea')
                      ...
                      : selectedItem.type === 'deliverable'
                        ? deliverableLabel
                      : '';
                    const selectedAreaAccent = (selectedItem.type === 'processArea' || selectedItem.type === 'deliverableProcessArea')
                    ...
                    const rawEffectiveArea = ((selectedItem.type === 'processArea' || selectedItem.type === 'deliverableProcessArea') ? selectedItem.area : selectedExecutionProcessArea || '').trim();
                    ...
                    if (isDesignBuildDeveloperPoolNode) {
                      ...
                      const scopedObjects = topLevelObjects.filter((item: any) => {
                        const processArea = String(item.processArea || '').trim();
                        return !processArea || getProcessAreasForProjectCycle(project.id, parentCycleId || undefined).includes(processArea);
                      });
                      const objectCount = scopedObjects.length;
                      ...
                      return (
                        <Box>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, mb: 1.5 }}>
                            ...
                            <Typography variant="caption" sx={{ color: deliverableAccent, fontWeight: 700 }}>Design and Build Estimation</Typography>
                            <Typography variant="caption" color="text.disabled">›</Typography>
                            <Typography variant="caption" sx={{ color: deliverableAccent, fontWeight: 700 }}>Developer Pool</Typography>
                          </Box>
                          ...
                          <Box sx={{ display: 'grid', gap: 1, gridTemplateColumns: { xs: '1fr', sm: 'repeat(4, minmax(140px, 1fr))' }, mb: 1.25 }}>
                            <Paper sx={{ p: 1, border: `1px solid ${deliverableAccent}44`, backgroundColor: 'rgba(255,255,255,0.04)' }}>
                              ...
                              <Typography variant="h6" sx={{ color: deliverableAccent }}>{objectCount}</Typography>
                            </Paper>
                            <Paper sx={{ p: 1, border: `1px solid ${deliverableAccent}44`, backgroundColor: 'rgba(255,255,255,0.04)' }}>
                              <Typography variant="caption" color="text.secondary">Recommended Developers</Typography>
                              <Typography variant="h6" sx={{ color: deliverableAccent }}>{recommendedTargetDevelopers || 0}</Typography>
                              <Typography variant="caption" color="text.secondary">Range {recommendedMinDevelopers || 0} - {recommendedMaxDevelopers || 0}</Typography>
                            </Paper>
                            <Paper sx={{ p: 1, border: `1px solid ${deliverableAccent}44`, backgroundColor: 'rgba(255,255,255,0.04)' }}>
                              <Typography variant="caption" color="text.secondary">Pool Size</Typography>
                              <Typography variant="h6" sx={{ color: deliverableAccent }}>{poolSize}</Typography>
                              <Typography variant="caption" color="text.secondary">{objectsPerDeveloper || 0} objects/developer</Typography>
                            </Paper>
                            <Paper sx={{ p: 1, border: `1px solid ${deliverableAccent}44`, backgroundColor: 'rgba(255,255,255,0.04)' }}>
                              <Typography variant="caption" color="text.secondary">Coverage</Typography>
                              <Chip
                                size="small"
                                label={poolCoverageState === 'balanced' ? 'Balanced' : poolCoverageState === 'under' ? 'Add Developers' : poolCoverageState === 'over' ? 'Pool Oversized' : 'Not Set'}
                                color={poolCoverageState === 'balanced' ? 'success' : poolCoverageState === 'under' ? 'warning' : 'default'}
                                variant="outlined"
                                sx={{ mt: 0.6 }}
                              />
                            </Paper>
                          ...
                          {poolCoverageState === 'over' && (
                            <Alert severity="info" sx={{ mb: 1.25 }}>
                              Current pool is larger than needed. Guidance is at least 5 objects per developer.
                            </Alert>
                          )}
                    ...
                    if (isDesignBuildEstimationDeliverable) {
                      ...
                      const objectRows = topLevelObjects.map((item: any) => {
                        const matches = hasRequiredFields(item)
                          ? matrixRows.filter((row) => (
                              row.buildType === item.buildType
                              && row.factorType === item.factorType
                      ...
                      const displayedProcessAreaRows = estimationSelectedArea
                        ? processAreaRows.filter((row) => row.area === estimationSelectedArea)
                        : processAreaRows;
                      ...
                      const translucentMetricCardSx = {
                        p: 1.25,
                        border: `1px solid ${estimationAccent}33`,
                      ...
                      return (
                        <Box>
                          ...
                          {!isEstimationProcessAreaSelection && (
                            <>
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                                <LinearProgress variant="determinate" value={completionPct} sx={{ width: 180, height: 6, borderRadius: 3, backgroundColor: 'rgba(255,255,255,0.22)', '& .MuiLinearProgress-bar': { backgroundColor: estimationAccent, borderRadius: 3 } }} />
                                <Typography variant="body2" sx={{ color: estimationAccent, fontWeight: 600 }}>{completionPct}% configured</Typography>
                              </Box>
                              ...
                              <Box sx={{ display: 'grid', gap: 1, gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, minmax(160px, 1fr))', lg: 'repeat(6, minmax(140px, 1fr))' }, mb: 2 }}>
                                <Paper sx={translucentMetricCardSx}>
                                  ...
                                  <Typography variant="h6" sx={{ mt: 0.25, fontWeight: 700, color: estimationAccent }}>{objectRows.length}</Typography>
                                </Paper>
                                <Paper sx={translucentMetricCardSx}>
                                ...
                                <Paper sx={{ ...translucentMetricCardSx, border: `1px solid ${estimationAccent}66` }}>
                          ...
                          {missingRequiredCount > 0 && (
                            <Alert severity="warning" sx={{ mb: 1.25, border: `1px solid ${estimationAccent}44`, backgroundColor: 'rgba(255,255,255,0.07)' }}>
                              {missingRequiredCount} object(s) are missing one or more required fields: Build Type, Factor Type, and Complexity.
                            </Alert>
                          )}
                          {unconfiguredCount > 0 && (
                            <Alert severity="info" sx={{ mb: 1.25, border: `1px solid ${estimationAccent}44`, backgroundColor: 'rgba(255,255,255,0.07)' }}>
                          ...
                          <Paper sx={{ ...translucentPanelSx, mb: 2 }}>
                            ...
                            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 90px 90px 90px 80px 90px', gap: 1, pb: 0.5, borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                              <Typography variant="caption" sx={{ color: estimationAccent, fontWeight: 700 }}>Process Area</Typography>
                              <Typography variant="caption" sx={{ color: estimationAccent, fontWeight: 700, textAlign: 'right' }}>Objects</Typography>
                              <Typography variant="caption" sx={{ color: estimationAccent, fontWeight: 700, textAlign: 'right' }}>Task Groups</Typography>
                              <Typography variant="caption" sx={{ color: estimationAccent, fontWeight: 700, textAlign: 'right' }}>Design Hrs</Typography>
                              <Typography variant="caption" sx={{ color: estimationAccent, fontWeight: 700, textAlign: 'right' }}>Build Hrs</Typography>
                              <Typography variant="caption" sx={{ color: estimationAccent, fontWeight: 700, textAlign: 'right' }}>Total Hrs</Typography>
                            </Box>
                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.35, mt: 0.5 }}>
                              {displayedProcessAreaRows.map((areaRow) => (
                                <Box key={areaRow.area} sx={{ display: 'grid', gridTemplateColumns: '1fr 90px 90px 90px 80px 90px', gap: 1, py: 0.35, borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                                  <Typography variant="body2" sx={{ fontWeight: 600, color: estimationAccent }}>{getProcessAreaDisplayName(project.id, areaRow.area, parentCycleId || undefined)}</Typography>
                                  ...
                                  <Typography variant="body2" sx={{ textAlign: 'right', fontWeight: 700, color: estimationAccent }}>{areaRow.hours.toFixed(2)}</Typography>
                                </Box>
                              ))}
                              {displayedProcessAreaRows.length === 0 && (
                          ...
                          {displayedProcessAreaRows.map((areaRow) => (
                            <Paper key={`area-objects-${areaRow.area}`} sx={{ ...translucentPanelSx, mb: 2 }}>
                              ...
                              <Box sx={{ display: 'grid', gridTemplateColumns: '1.1fr 0.7fr 1.3fr 0.7fr 0.6fr 0.6fr 0.6fr', gap: 1, pb: 0.5, borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                                ...
                                <Typography variant="caption" sx={{ color: estimationAccent, fontWeight: 700 }}>Factor Type</Typography>
                                <Typography variant="caption" sx={{ color: estimationAccent, fontWeight: 700 }}>Complexity</Typography>
                                <Typography variant="caption" sx={{ color: estimationAccent, fontWeight: 700, textAlign: 'right' }}>Design</Typography>
                                <Typography variant="caption" sx={{ color: estimationAccent, fontWeight: 700, textAlign: 'right' }}>Build</Typography>
                                <Typography variant="caption" sx={{ color: estimationAccent, fontWeight: 700, textAlign: 'right' }}>Total</Typography>
                              </Box>
                              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.45, mt: 0.5 }}>
                                {areaRow.rows.map((row) => (
                                  <Box key={row.id} sx={{ display: 'grid', gridTemplateColumns: '1.1fr 0.7fr 1.3fr 0.7fr 0.6fr 0.6fr 0.6fr', gap: 1, py: 0.35, borderBottom: '1px solid rgba(255,255,255,0.04)', alignItems: 'center' }}>
                                    ...
                                    <Typography variant="body2" sx={{ textAlign: 'right', fontWeight: 700, color: row.hours > 0 ? estimationAccent : 'text.secondary' }}>
                          ...
                          <Paper sx={translucentPanelSx}>
                            ...
                            {Object.keys(taskTotals).length === 0 ? (
                              ...
                              <Box sx={{ display: 'grid', gap: 0.5 }}>
                                {Object.entries(taskTotals).sort(([a], [b]) => a.localeCompare(b)).map(([taskName, hours]) => (
                                  <Box key={taskName} sx={{ display: 'flex', justifyContent: 'space-between', py: 0.35, borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                                    <Typography variant="body2">{taskName}</Typography>
                                    <Typography variant="body2" sx={{ fontWeight: 700, color: estimationAccent }}>{hours.toFixed(2)}</Typography>
                                  </Box>
                                ))}
                              </Box>
                            )}
                    ...
                    if (isProjectRoadmapSummaryDeliverable || isMockCriteriaSummaryDeliverable || isMockCriteriaCycleNode || isMockCriteriaApprovalsNode) {
                      ...
                      if (isMockCriteriaSummaryDeliverable) {
                          ...
                          const criteriaCycles = Array.from(criteriaCycleMap.values()).sort((a: any, b: any) => {
                            ...
                            return String(a.testPhase || a.description || a.name || '').localeCompare(String(b.testPhase || b.description || b.name || ''));
                          });
                          ...
                          renderedInventoryItems.map((item, idx) => {
                            ...
                            return (
                              <React.Fragment key={item.id}>
                                ...
                                <Box sx={{ p: 0.6, borderBottom: '1px solid rgba(255,255,255,0.06)', backgroundColor: rowBg }}>
                                  {isSubObject ? (
                                    <Typography sx={{ fontSize: '0.78rem', color: 'rgba(255,255,255,0.75)', px: 0.4, py: 0.4 }}>
                                      {item.processArea || '—'}
                                    </Typography>
                                  ) : (
          ...
          {/* Roadmap Tab Content */}
          {sectionMode === 'planning' && tabValue === 7 && (
            <RoadmapView
              programs={programs}
              mockCycles={mockCycles}
              projectsByMockCycle={projectsByMockCycle}
        ...
        <DialogContent sx={{ pt: 2, maxHeight: '70vh', overflowY: 'auto', px: 3 }}>
          <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2, mt: 2 }}>
            ...
            <TextField
              select
              fullWidth
              label={invDialogAppOptions.length === 0 ? 'Source Application (none linked)' : 'Source Application'}
              value={invDialogSourceAppId}
              onChange={(e) => {
                setInvDialogSourceAppId(e.target.value);
              }}
              disabled={editingInventoryItemId !== null || invDialogAppOptions.length === 0}
              variant="outlined"
              size="small"
            ...
            onClick={async () => {
              ...
              try {
                ...
                if (editingInventoryItemId) {
                  const selectedTargetAppOption = invDialogAppOptions.find((opt: any) => opt.id === invDialogAppId) || null;
                  const selectedSourceAppOption = invDialogAppOptions.find((opt: any) => opt.id === invDialogSourceAppId) || null;
                  const targetApplicationId = selectedTargetAppOption?.applicationId || null;
                  const sourceApplicationId = selectedSourceAppOption?.applicationId || null;
                  ...
                  setProjectInventoryItems(projectInventoryItems.map(item =>
                    item.id === editingInventoryItemId 
                      ? {
                          ...
                          targetApplicationName: selectedTargetAppOption?.name || '',
                          sourceApplicationId: sourceApplicationId || '',
                          sourceApplicationName: selectedSourceAppOption?.name || '',
                  ...
                  const newItem: any = {
                    ...
                    targetApplicationName: apiData.targetApplicationName || selectedTargetAppOption?.name || '',
                    sourceApplicationId: apiData.sourceApplicationId || sourceApplicationId || '',
                    sourceApplicationName: apiData.sourceApplicationName || selectedSourceAppOption?.name || '',
                  ...
                  for (const so of invDialogSubObjects) {
                    try {
                      ...
                      newItems.push({
                        ...
                        targetApplicationName: sub.targetApplicationName || selectedTargetAppOption?.name || '',
                        sourceApplicationId: sub.sourceApplicationId || sourceApplicationId || '',
                        sourceApplicationName: sub.sourceApplicationName || selectedSourceAppOption?.name || '',
                        processArea: sub.processArea,
                        complexity: null, deploymentDisposition: null, buildType: null,
                        objectType: null, cutoverPhase: null, ddmApproach: null,
                        riskSecurityType: null, migrationType: null, factorType: null, loadMethod: null,
