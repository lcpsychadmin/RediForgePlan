...
const ObjectApplicationsPage: React.FC = () => {
  ...
  const [setupPanelOpen, setSetupPanelOpen] = React.useState(false);
  ...
  return (
    <Layout>
      <Box sx={{ p: 3 }}>
        ...
        <Accordion
          expanded={setupPanelOpen}
          onChange={(_event, expanded) => setSetupPanelOpen(expanded)}
          sx={{ mb: 2, backgroundColor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.12)', '&:before': { display: 'none' } }}
        >
          <AccordionSummary expandIcon={<ExpandMoreIcon />}>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', pr: 1 }}>
              <Typography sx={{ fontWeight: 700 }}>Application Setup</Typography>
              <Typography variant="caption" color="text.secondary">Linked applications: {linked.length}</Typography>
            </Box>
          </AccordionSummary>
          <AccordionDetails>
            <Stack spacing={2}>
              {isLoadingSubObjects ? (
                ...
                <Alert severity="info">This object has no sub-objects. The object itself is used as the data-definition scope.</Alert>
              )}
              ...
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
                ...
                <Button variant="contained" onClick={handleLink} sx={{ textTransform: 'none' }} disabled={!linkAppId}>Link Application</Button>
              </Stack>
          ...
          <Card sx={{ mt: 2, backgroundColor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.12)' }}>
            <CardContent>
              ...
              <Box sx={{ mb: 2, p: 1.25, borderRadius: 1, border: '1px solid rgba(255,255,255,0.12)', backgroundColor: 'rgba(255,255,255,0.02)' }}>
                ...
                <Typography sx={{ fontWeight: 700, fontSize: '0.78rem', color: 'text.secondary', mt: 1.5, mb: 1 }}>Pull Metadata from Databricks</Typography>
