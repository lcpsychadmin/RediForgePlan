...
const ObjectInventoryCatalogPage: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!active) return;
        setError(e?.response?.data?.message || 'Failed to load object catalog.');
        setObjects([]);
      } finally {
          ...
          {objects.length === 0 ? (
            ...
            objects.map((obj, idx) => {
              ...
              return (
                ...
                >
                  <TableCell className="table-cell table-cell-object-id">{objectCode}</TableCell>
                  <TableCell className="table-cell">{obj.description || '-'}</TableCell>
                  <TableCell className="table-cell">{obj.processArea || obj.process_area || '-'}</TableCell>
...
export default ObjectInventoryCatalogPage;
